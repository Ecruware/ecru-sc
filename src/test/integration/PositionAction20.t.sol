...
contract PositionAction20Test is IntegrationTestBase {
    ...
    function test_delegateViaStablecoin() public {
        uint256 depositAmount = 10_000*1 ether;
        uint256 creditAmount = 5_000*1 ether;
        ...
        // delegate via stablecoin
        vm.startPrank(user);
        stablecoin.approve(address(userProxy), creditAmount);
        userProxy.execute(
            address(positionAction),
            abi.encodeWithSelector(
                positionAction.delegateViaStablecoin.selector,
                user, // creditor
                daiVault, // delegateVault
                creditAmount, // credit
                emptySwap,
    ...
    function test_delegateViaStablecoin_with_USDC() public {
        uint256 depositAmount = 10_000*1 ether;
        uint256 creditAmount = 5_000*1 ether;
        ...
        // get rid of borrowed stablecoin for testing purposes
        vm.prank(user);
        stablecoin.transfer(address(0x1), creditAmount);
        ...
        uint256 expectedAmountOut;
        {
            address[] memory assetPath = new address[](2);
            assetPath[0] = address(USDC);
            assetPath[1] = address(stablecoin);
    ...
    function test_delegateViaStablecoin_with_USDT_EXACT_OUT() public {
        uint256 depositAmount = 10_000*1e6;
        uint256 creditAmount = 5_000*1 ether;
        uint256 delegateAmount = 1_000*1e6;
        ...
        {
            address[] memory assetPath = new address[](2);
            assetPath[0] = address(stablecoin);
            assetPath[1] = address(USDT);

            expectedAmountOut = delegateAmount * 99e12/100; // expected amount of stablecoin out
            auxSwap = SwapParams({
                swapProtocol: SwapProtocol.BALANCER,
                swapType: SwapType.EXACT_OUT,
                assetIn: address(USDT),
    ...
    function test_repayAndClaim() public {
        ...
        uint256 borrowAmount = 2_000*1 ether; // stablecoin to borrow
        uint256 half = borrowAmount/2; // stablecoin/credit to delegate

        // set up initial position
        _depositAndBorrow(userProxy, address(daiVault), depositAmount, borrowAmount);

        // delegate a portion of the stablecoin borrowed
        vm.startPrank(user);
        stablecoin.approve(address(userProxy), half);
        userProxy.execute(
            address(positionAction),
            abi.encodeWithSelector(
                positionAction.delegateViaStablecoin.selector,
        ...
        deal(address(stablecoin), user, halfDebt); // update stablecoin balance to pay back interest accrued

        // repay stablecoin borrowed and undelegate credit that was delegated
        vm.startPrank(user);
        stablecoin.approve(address(userProxy), halfDebt);
        userProxy.execute(
            address(positionAction),
            abi.encodeWithSelector(
    ...
    function test_repayAndClaim_with_interest() public {
        uint256 depositAmount = 10_000*1 ether; // dai to deposit
        uint256 borrowAmount = 2_000*1 ether; // stablecoin to borrow
        uint256 halfNormalDebt = borrowAmount/2; // stablecoin/credit to delegate
        ...
        // delegate a portion of the stablecoin borrowed
        vm.startPrank(user);
        stablecoin.approve(address(userProxy), halfNormalDebt);
        userProxy.execute(
            address(positionAction),
            abi.encodeWithSelector(
                positionAction.delegateViaStablecoin.selector,
                user, // creditor
                daiVault, // vault
                halfNormalDebt, // delegate half of stablecoin borrowed
                emptySwap,
                emptyPermitParams
            )
        );
        ...
        deal(address(stablecoin), user, debt/2);
        ...
        // repay stablecoin borrowed and claim undelegated credit
        vm.startPrank(user);
        stablecoin.approve(address(userProxy), debt/2);
        userProxy.execute(
            address(positionAction),
            abi.encodeWithSelector(
                positionAction.repayAndClaim.selector,
    ...
    function test_repayAndClaim_USDT_swap_EXACT_OUT_with_interest() public {
        ...
        // repay stablecoin borrowed and undelegate credit that was delegated
        vm.startPrank(user);
        stablecoin.transfer(address(0x1), halfNormalDebt); // get rid of stablecoin
        ...
        assertEq(stablecoin.balanceOf(user), 0); // verify no stablecoin left over
    ...
    function test_repayAndClaim_DAI_swap_EXACT_OUT() public {
        ...
        uint256 amountInMax = halfDebt * 101 / 100;
        {
            ...
            creditParams = CreditParams({
                ...
                auxSwap: SwapParams({
                    ...
                    swapType: SwapType.EXACT_OUT,
                    assetIn: address(DAI),
                    amount: halfDebt, // amountOut in Stablecoin
        ...
        // repay stablecoin borrowed and undelegate credit that was delegated
        vm.startPrank(user);
        stablecoin.transfer(address(0x1), halfNormalDebt); // get rid of stablecoin and replace it with USDT
    ...
    function _undelegateAndWarp(
        ...
        vm.warp(block.timestamp + (CDPVault(vault).EPOCH_FIX_DELAY() * CDPVault(vault).EPOCH_DURATION()));
        ...
        uint256 secondsToClaim = (claimableAtEpoch - epoch) * CDPVault(vault).EPOCH_DURATION();
